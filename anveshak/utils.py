"""Small reusable helpers for parsing, ranking, and reproducibility."""
...
def tokenize_for_bm25(text: str) -> list[str]:
    """Lowercase and tokenize text for the lightweight lexical scorer."""

    return [token.lower() for token in TOKEN_RE.findall(text)]


def min_max_normalize(values: list[float]) -> list[float]:
    """Scale a list of scores into the ``[0, 1]`` interval."""
...
def apply_global_seed(seed: int) -> None:
    """Apply one startup seed across Python, NumPy, PyTorch, and Transformers."""
...
def bm25_scores(
    ...
    *,
    ...
    """Compute simple BM25-style lexical scores for small in-process corpora."""
