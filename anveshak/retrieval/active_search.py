"""Active multi-round web retrieval orchestration."""
...
class SearchPlan:
    """Structured instructions for a live retrieval run."""
...
@dataclass(slots=True)
class SearchRound:
    """One executed search round and the evidence it returned."""
...
class ActiveSearchOrchestrator:
    """Execute retrieval plans and optionally ask the model for follow-up rounds."""
    ...
    def run(
        ...
        *,
        ...
        """Execute a search plan until its budget or follow-up logic is exhausted."""
        ...
        for round_index in range(1, max(plan.max_rounds, 1) + 1):
            ...
            for query in current_queries[:3]:
                try:
                    ...
                    handle.emit(
                        "warning",
                        text=f'Active web search skipped one query after a retrieval failure: "{query}" ({exc})',
                    )
                    continue
...
def _dedupe_chunks(chunks: list[RetrievedChunk]) -> list[RetrievedChunk]:
    """Drop near-duplicate web chunks while keeping the highest-score copy."""
